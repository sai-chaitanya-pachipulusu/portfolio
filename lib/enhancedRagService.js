...
class EnhancedRAGService {
  ...
  async search(query, options = {}) {
    ...
    try {
      ...
      console.log('Graph RAG search successful');
      return {
        context: searchResult.context,
        sources: searchResult.sources,
        confidence: searchResult.confidence,
        searchType: 'graph-rag',
        resultCount: searchResult.resultCount
      };

      // Cache the result
      this.cache.set(cacheKey, searchResult, 60 * 60 * 1000); // 1 hour cache

    } catch (error) {
      console.warn('Graph RAG failed, using standard RAG:', error.message);
      //return this.getFallbackResponse(query);

      // Fallback to your working RAG service
      const fallbackResult = await this.ragFallback.search(query, options);
      
      return {
        context: fallbackResult.context,
        sources: fallbackResult.sources,
        confidence: fallbackResult.confidence,
        searchType: 'standard-rag-fallback',
        resultCount: 1
      };
    }
  }
