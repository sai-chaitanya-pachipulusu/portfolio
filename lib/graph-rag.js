  ...
  async loadData() {
    ...
    try {
      ...
      const [embeddingsData, sourcesData, graphData] = await Promise.all([
        fs.readFile(this.paths.embeddings, 'utf-8').then(JSON.parse),
        fs.readFile(this.paths.sources, 'utf-8').then(JSON.parse),
        fs.readFile(this.paths.graph, 'utf-8').then(JSON.parse)
      ]);
      ...
      console.log(`Loaded: ${Object.keys(this.embeddingsCache).length} embeddings, ${this.graphCache.nodes.size} nodes`);
      this.isInitialized = true;
      
      return true;
    } catch (error) {
      if (error.code === 'ENOENT') {
        console.error('RAG data files not found. Please run initialization first.');
      } else {
  ...
  async search(query, options = {}) {
    ...
    try {
      ...
      if (!this.isInitialized) {
        ...
        if (!loaded) {
          throw new Error('Could not load RAG data');
        }
      }
      ...
      const semanticResults = this.findSimilarNodes(queryEmbedding, maxResults, similarityThreshold);

      let finalResults = semanticResults;

      // Enhance with graph traversal
      if (includeGraphTraversal && this.graphCache) {
      ...
      return {
        ...
        hasGraphEnhancement: includeGraphTraversal
      };
...
const graphRAGSystem = new GraphRAGSystem();
