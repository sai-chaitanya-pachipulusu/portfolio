  ...
  async loadData() {
    ...
    try {
      ...
      const [embeddingsData, sourcesData, graphData] = await Promise.all([
        fs.readFile(path.join(this.dataDir, 'embeddings.json'), 'utf-8').then(JSON.parse),
        fs.readFile(path.join(this.dataDir, 'processed_sources.json'), 'utf-8').then(JSON.parse),
        fs.readFile(path.join(this.dataDir, 'graph.json'), 'utf-8').then(JSON.parse)
      ]);
